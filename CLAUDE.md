...
All of these are listed in `.gitignore`. The modeler is a **frontend-only consumer** — the umbrella repo (`digitalhome-cloud-darkfactory`) owns the single Amplify backend. Per-repo `amplify/` and `src/aws-exports.js` are **symlinks** created by `scripts/sync-env.sh`.
...
The umbrella repo owns the Amplify Gen1 backend (symlinked into each app by `scripts/sync-env.sh`). The semantic-core ontology files live inside the modeler repo under `semantic-core/`.
